...
# 3. 读取数据
cat("正在读取数据文件...\n")
raw_data <- read.csv("PCA_KB_WGJ.csv", row.names = 1, check.names = FALSE)
...
group <- read.csv("group_KB_WGJ.csv", row.names = 1)
...
ggsave(filename = "PCA_WGJ_CK.pdf", plot = p_pca, width = 6, height = 6, device = cairo_pdf)
...
#OPLS-DA_WGJ_CK_permutation.pdf"
plot(perm_result, typeVc = "summary")
#"OPLS-DA_WGJ_CK_summary.pdf"
...
ggsave("OPLS-DA_WGJ_CK_CV.pdf", p_oplsda, width=6, height=6, device=cairo_pdf)
...
fit <- eBayes(fit)
limma_results <- topTable(fit, coef=2, number=Inf, adjust.method="fdr")
sum(limma_results$adj.P.Val < 0.05)  # 通常比t-test + FDR多
...
# 保存完整结果
write.csv(data_VIP_P_data, "OPLSDA_VIP_P_WGJ_CK_complete.csv", row.names = FALSE)
...
write.csv(data_VIP_P_select, "OPLSDA_VIP_P_WGJ_CK_select.csv", row.names = FALSE)
