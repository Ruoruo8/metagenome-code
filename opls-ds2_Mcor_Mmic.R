...
# 3. 读取数据
cat("正在读取数据文件...\n")
raw_data <- read.csv("PCA_JZL_WGJ.csv", row.names = 1, check.names = FALSE)
...
group <- read.csv("group_JZL_WGJ.csv", row.names = 1)
...
ggsave(filename = "PCA_WGJ_JZL.pdf", plot = p_pca, width = 6, height = 6, device = cairo_pdf)
...
#OPLS-DA_WGJ_JZL_permutation.pdf"
plot(perm_result, typeVc = "summary")
#"OPLS-DA_WGJ_JZL_summary.pdf"
...
ggsave("OPLS-DA_WGJ_JZL_CV.pdf", p_oplsda, width=6, height=6, device=cairo_pdf)
...
# 保存完整结果
write.csv(data_VIP_P_data, "OPLSDA_VIP_P_WGJ_JZL_complete.csv", row.names = FALSE)
...
write.csv(data_VIP_P_select, "OPLSDA_VIP_P_WGJ_JZL_select.csv", row.names = FALSE)
